    ...
    */
    computeAABB(){
		...
		for (var i = 0; i < this.numVertices; i++) {
			...
			for (var j = 0; j < 3; j++) {
				this.minXYZ[j] = Math.min(this.minXYZ[j], vertex[j]);
				this.maxXYZ[j] = Math.max(this.maxXYZ[j], vertex[j]);
			}
		}
    }
    ...
    loadFromOBJ(fileText)
    {
		...
		for (var i = 0; i < fileLines.length; i++) {
			...
			if (fileLines[i][0] == "#") {
				...
				var tokens = fileLines[i].split(/\b\s+(?!$)/);
				if (tokens[0] == "v") {
					// Handle vertex
					...
					// Handle triangles
					for (var j = 1; j < tokens.length; j++) {
						this.fBuffer.push(parseInt(tokens[j]-1));
					}
				}
			}
		}
